...
function renderMetrics() {
  const inRange = currentProfiles.filter((profile) => !profile.followers || (profile.followers >= Number(elements.minFollowers.value) && profile.followers <= Number(elements.maxFollowers.value))).length;
  const course = currentProfiles.filter((profile) => profile.tags.some((tag) => ["線上課程", "顧問服務", "講座課程", "求職課程"].includes(tag))).length;
  const priority = Object.values(savedState).filter((item) => item.pipeline === "priority").length;
...
function renderCards() {
  elements.cardGrid.innerHTML = currentProfiles.map((profile) => {
