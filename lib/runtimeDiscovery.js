...
const MAX_DISCOVERED_PROFILES = 24;
const MAX_PROFILE_ENRICH = 18;
    ...
    await Promise.all(
      expansionTargets.map(async (url) => {
        try {
          ...
          crawlLog.warnings.push(`無法展開搜尋結果頁：${url}`);
        }
      }),
    );
...
async function enrichWebsite(candidate, crawlLog) {
  ...
  try {
    ...
    crawlLog.warnings.push(`無法讀取外部網站：${candidate.website}`);
    return {
      website: candidate.website,
      title: "",
      description: "",
...
async function enrichProfile(candidate, input, crawlLog) {
  try {
    ...
    crawlLog.warnings.push(`無法讀取公開個人頁：${candidate.url}`);
...
async function discoverCreators(rawInput = DEFAULT_DISCOVERY_INPUT) {
  ...
  for (const seed of baseSeedProfiles) {
    ...
    candidateMap.set(`${seed.platform}:${seed.handle}`, { ...seed });
  }
  ...
  for (const url of discoveredUrls) {
    ...
    if (!candidateMap.has(key)) {
      const queryHit =
        crawlLog.queries.find((item) => url.includes("instagram.com/") ? item.query.includes("instagram.com") : item.query.includes("threads.net"))?.query ||
        "";
      candidateMap.set(key, {
        ...normalized,
  ...
  const seedKeys = new Set(baseSeedProfiles.map((profile) => `${profile.platform}:${profile.handle}`));
  ...
  const payload = {
    ...
    crawlSummary: {
      ...
      seedProfiles: [...candidateMap.values()].filter((candidate) =>
        seedKeys.has(`${candidate.platform}:${candidate.handle}`),
      ).length,
