  ...
  const discoveredUrls = (
    await Promise.all(
      queries.map(async (query) => {
        try {
          return await searchDuckDuckGo(query, crawlLog);
        } catch (error) {
          pushWarning(
            crawlLog,
            `DuckDuckGo 搜尋未回應（${query}）：${error.message || error}`,
...
async function enrichSingleProfile(rawUrl, rawInput = {}) {
  const normalized = normalizeCandidateUrl(rawUrl);
  if (!normalized) {
    throw new Error("無法解析這個網址，請貼 IG 或 Threads 個人頁網址");
  }

  const input = normalizeInput(rawInput);
  const crawlLog = { queries: [], warnings: [], suppressedWarnings: 0 };
  const enriched = await enrichProfile(
    { ...normalized, sourceNotes: ["手動加入"] },
    input,
    crawlLog,
  );
